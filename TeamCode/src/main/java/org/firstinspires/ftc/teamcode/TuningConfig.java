...
/**
 * Tuning configuration for the FtcAutoTune library's OpModes
 * ({@code PID Auto Tuner (Position)} and {@code PIDF Auto Tuner (Velocity)}),
 * which come from the {@code com.github.aaravdhawan25.FtcAutoTune} JitPack
 * dependency.
 *
 * <p>This class is found via reflection at runtime by those OpModes -- the
 * package ({@code org.firstinspires.ftc.teamcode}) and class name
 * ({@code TuningConfig}) must match exactly. Any field you omit falls back
 * to a built-in default in the library.
 */
